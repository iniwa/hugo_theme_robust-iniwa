...
Preserve this branch's Hugo templates, assets, optional `Site.Params` compatibility, and escaping. Where already present, retain Hugo Pipes, split CSS, plain JavaScript, DOMPurify in `load-memos.js`, SRI, and upstream markers `[mod]`, `[new]`, `[removed]`; this older branch does not acquire those implementations through a documentation update. Update the relevant README description whenever theme files change. Parent-specific decisions belong in the parent repository; do not change parent pins from this checkout.
