...
関する作業である。テーマリポジトリから値や方針を推測せず、親サイトごとの承認済みスコープと
運営者の明示判断を得てから実施する。
...
- [ ] **【中】広告方式を段階検証する（親サイト側）**
  ...
  - 状態: テーマ側の共通 gate、手動広告 partial、CLS 予約領域は完成。親サイトの方式選択、
    AdSense アカウント操作、公開テストは未承認のため未実施。
- [ ] **【高】プライバシーポリシー・運営者情報を再監査する（親サイト側）**
  ...
  - 調査時点で `iniwach.com/privacy/` は `no_ads` 未設定のため、親リポジトリの承認済み変更で
    front matter を設定する。法務・運用文面はテーマに置かない。
...
- [ ] **【高】同意管理（CMP）または広告の地域制限を決定する**
  - EEA / UK / スイスで Google 広告を配信する場合の認定 TCF CMP と、対象地域への広告配信を
    制限する選択肢を比較し、運営者が一方を明示的に決定する。
  - 決定とアカウント設定は `ADSENSE_PLAN.md` に記録する。テーマへ同意ロジックや地域判断を
    暗黙に組み込まない。
